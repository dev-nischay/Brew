...
export const userSignup = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (alreadyExists) return next(new AppError("User already exists", HttpStatus.Forbidden));
...
export const userSignin = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!isUser) return next(new AppError("User account not found. Please sign up to continue.", HttpStatus.NotFound));

  const compare = await bcrypt.compare(password, isUser.password);

  if (!compare) return next(new AppError("Incorrect Password", HttpStatus.BadRequest));
